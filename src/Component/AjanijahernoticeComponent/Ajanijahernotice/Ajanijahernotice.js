...
import { DataTable, List, Button, Provider, Divider } from 'react-native-paper';
import AsyncStorage from '@react-native-async-storage/async-storage';
// import BottomSheet from '../../AjanijahernoticeComponent/Ajanijahernotice/Bottomnav';
import Ionicons from 'react-native-vector-icons/Ionicons';
import { OrientationLocker, PORTRAIT } from 'react-native-orientation-locker';
// import {bootstrapAsync} from '../../../Navigation/index';
// import NavigationService from '../../../helpers/NavigationService';

const Areaalert = ({ route, navigation }) => {
  ...
  //     if (isUserActive) {
  //       NavigationService.navigate('Ajanijahernotice');
  //     } else {
  ...
  const tableData = async () => {
    ...
    // console.log('DATAFA', selectedDate);
    setIsLoading(true);
    setRefreshing(false);
    // setN(selectedDate);
    const { selectedDat } = route.params;
    ...
    fetch(
      `https://qaapi.jahernotice.com/api2/app/ajanijahernotice/dashboard/${Userid}/${selectedDat}`,
    )
      ...
      .then(data => {
        ...
        if (data.data.status == 0) {
          ...
          console.log('data.data.stateData', data.data.stateData);
        }
        setIsLoading(false);
      })
  ...
  const handlePress = message => {
    ...
    if (dateMatch) {
      navigation.navigate('AjanijahernoticLastPublishe', {
        Date: N ? N : '1',
      });
    }
  };
        ...
        <SafeAreaView style={styles.Container}>
          ...
          >
            {/* <View
              style={{
                flexDirection: 'row',
                backgroundColor: theme === 'LIGHT' ? '#efefef' : '#696969',
              }}>
              <View
                style={{
                  justifyContent: 'center',
                        ...
                        <>
                          ...
                          >
                            <Text
                              ...
                              //       'There are no new Ajani Jahernotice found. Click to view the last Area Alert received on',
                              //     )
                              //   ) {
                              //     navigation.navigate(
                              //       'AjanijahernoticLastPublishe',
                ...
                <ScrollView>
                  <View>
                    ...
                    <TouchableOpacity
                      activeOpacity={0.6}
                      onPress={() => ThisWeek()}>
                      <Text style={styles.bottomtext}>This Week</Text>
                    </TouchableOpacity>
                    <Divider style={{height: 1}} />
                    <TouchableOpacity
                      activeOpacity={0.6}
                      onPress={() => Last30Days()}>
                      <Text style={styles.bottomtext}>Last 30 Days</Text>
                    </TouchableOpacity>
                    <Divider style={{height: 1}} />
                    <TouchableOpacity
                      activeOpacity={0.6}
                      onPress={() => Last3Months()}>
                      <Text style={styles.bottomtext}>Last 3 Months</Text>
                    </TouchableOpacity>
                    <Divider style={{height: 1}} />
                    <TouchableOpacity
                      activeOpacity={0.6}
                      onPress={() => Last6Months()}>
                      <Text style={styles.bottomtext}>Last 6 Months</Text>
                    </TouchableOpacity>
                    <Divider style={{height: 1}} />
                    <TouchableOpacity
                      activeOpacity={0.6}
                      onPress={() => Last1Year()}>
                      <Text style={styles.bottomtext}>Last one Year</Text>
                    </TouchableOpacity>
                  </View>
                </ScrollView>
              </BottomSheet>
