...
const Viewcom = ({ route, navigation }) => {
  ...
  const [PageNumber, setPageNumber] = useState(500);

  // useEffect(() => {
  //   bootstrapAsync().then(({isUserActive}) => {
  //     if (isUserActive) {
  //       NavigationService.navigate('TPServiceCount');
  //     } else {
  ...
  const tableData = async () => {
    ...
    const { selectedDat, otherParam } = route.params;
    ...
    try {
      const response = await fetch(
        `https://qaapi.jahernotice.com/api2/getTP/EmaillogByUserID/${Userid}?PageSize=${PageNumber}&PageNo=${currentPage}&Dayif=${selectedDat}`,
        {
          method: 'GET',
          headers: myHeaders,
          redirect: 'follow',
                      ...
                      >
                            ...
                            >
                              <TouchableOpacity>
                                <Text
                                  ...
                                  onPress={() =>
                                    navigation.navigate(
                                      'TownPlanningServicesViewNotice',
                                      {
                                        imagepepar: `https://qaapi.jahernotice.com/${item.image_path}`,
                                        Notice_Date: formatDate(item.IssueDate),
                                        notifyby: item.ExistingOrNewTP,
                                        imagedownlod: `https://qaapi.jahernotice.com/${item.image_path}`,
                                        fileUrl: `https://qaapi.jahernotice.com/${item.image_path}`,
                                      },
                                    )
                                  }
                                >
                        ...
                        <View style={style.containerma}>
                          ...
                          <View style={style.VewText}>
                            ...
                            >
                              TP No.:{' '}
                              <Text style={style.textD}>{item.TPNO}</Text>
                            </Text>
                          </View>
                          <Divider style={style.ViewT} />
                          <View style={style.VewText}>
                          ...
                          <View style={style.VewText}>
                            ...
                            >
                              Taluka Name :{' '}
                              <Text style={style.textD}>{item.TalukaName}</Text>
                            </Text>
                          </View>
                          <Divider style={style.ViewT} />
                          <View style={style.VewText}>
