...
const Viewimg = ({ route, navigation }) => {
  ...
  //     if (isUserActive) {
  //       NavigationService.navigate('TaitleSearchimg');
  //     } else {
  ...
  useEffect(() => {
    const { imagepepararia, publish_date, notifysource, otherParam } =
      route.params;
    replaceSpacesInImageUrl(imagepepararia);
    setpublish_date(publish_date);
    setnotifysource(notifysource);
    console.log(imagepepararia);
  }, []);
  ...
  const replaceSpacesInImageUrl = imageUrl => {
    ...
    if (imageUrl && imageUrl.includes(' ')) {
      ...
      setimagepeparariaa(img);
    } else {
      const imge = imageUrl;
      setimagepeparariaa(imge);
    }
