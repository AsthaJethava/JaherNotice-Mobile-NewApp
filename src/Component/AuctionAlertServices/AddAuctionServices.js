...
function AddTPServices({ route, navigation }) {
  ...
  const requestData = async () => {
    ...
    fetch(`https://qaapi.jahernotice.com/api/req/add/auction`, {
      method: 'POST',
      headers: {
  ...
  // console.log('ServicesubTypeID', ServicesubTypeID);
  // console.log('DistrictId', DistrictId);
  // console.log('TalukaId', TalukaId);
  // console.log('VillageId', VillageId);
  // console.log('DistrictNamea', DistrictNamea);
  // console.log('TalukaNamea', TalukaNamea);
  // console.log('VillageNamea', VillageNamea);
  // console.log('SubType_Namea', SubType_Namea);
        ...
        <View style={styles.container}>
          <>
            ...
            <SelectList
              ...
              placeholder={
                selectedStatea ? selectedStatea : 'Select State Name*'
              }
              boxStyles={{
                // marginBottom: 10,
                borderRadius: 5,
              ...
              // }}
              onSearch={search => {
                const filteredData = datasa.filter(
                  item =>
                    item.keya &&
                    item.toLowerCase().includes(search.toLowerCase()),
                );
                ...
                if (filteredData.length > 0) {
                  ...
                  return [{ keya: 'No Data Found', label: 'No Data Found' }];
                }
              }}
            />
            ...
            Servicesubname === 'Per Village' ? (
              <>
                ...
                <SelectList
                  ...
                  // }}
                  onSearch={search => {
                    const filteredData = Taluka.filter(
                      item =>
                        item.key &&
                        item.toLowerCase().includes(search.toLowerCase()),
                    );
                    ...
                    if (filteredData.length > 0) {
                      ...
                      return [{ key: 'No Data Found', label: 'No Data Found' }];
                    }
                  }}
                />
              </>
